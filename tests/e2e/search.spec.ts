...
test.describe("Search (/search)", () => {
  ...
  test("a suburb chip with zero real businesses does not render", async ({ page }) => {
    ...
    // every remaining category now has ≥1 real business. The underlying
    // filtering logic is still fully covered by searchService.test.ts's
    // synthetic-fixture unit tests for categoriesWithBusinesses.
    const search = new SearchPage(page);
    await search.goto();

    await expect(search.categoryChip("Box Hill")).toHaveCount(0);
  });
