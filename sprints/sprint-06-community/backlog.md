...
|----|------|------------|----------|--------|
| B-001 | Activate the (Future) Event Schema from JSON_SCHEMA.md as current; seed `data/events.json` with sample records | Sprint 1 repository layer | High | Not Started |
| B-002 | Activate the (Future) Promotion Schema from JSON_SCHEMA.md as current; seed `data/promotions.json` with sample records | Sprint 1 repository layer | High | Not Started |
| B-003 | Define a new minimal Announcement schema (id, title, message, publishedAt, expiresAt optional, priority/type) per JSON_SCHEMA.md conventions; seed `data/announcements.json` | Sprint 1 repository layer | High | Not Started |
| B-004 | Build `EventRepository` and `PromotionRepository` following the existing `BusinessRepository` pattern | B-001, B-002 | High | Not Started |
| B-005 | Build `AnnouncementRepository` | B-003 | High | Not Started |
| B-006 | Build community events section (upcoming events, image, location, dates) | B-004 | High | Not Started |
| B-007 | Build promotions section, linking each promotion to its business via `businessId` and the Sprint 4 business detail page | B-004 | High | Not Started |
| B-008 | Build announcements section (noticeboard-style, newest first, expiry-aware) | B-005 | Medium | Not Started |
| B-009 | Build the unified featured content mechanism (aggregates `featured` items across events, promotions, announcements) | B-006, B-007, B-008 | Medium | Not Started |
| B-010 | Build community spotlight (highlights one featured business or event; reuses existing Business/Event data) | B-009 | Medium | Not Started |
| B-011 | Build local news placeholder ("coming soon" section, no feed logic) | None | Low | Not Started |
| B-012 | Integrate all sections into the homepage layout | B-006–B-011 | High | Not Started |
| B-013 | Responsive pass across all six sections | B-012 | High | Not Started |
| B-014 | Accessibility pass (alt text, heading hierarchy, keyboard access) | B-012 | High | Not Started |
| B-015 | Playwright: community page renders all sections; promotion links to correct business; empty states render correctly | B-012 | High | Not Started |
