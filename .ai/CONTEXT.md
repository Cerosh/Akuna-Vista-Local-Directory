...
All business data will remain static until Version 2. Current dataset (18 real businesses, 18 categories, 5 suburbs, 1 real event, 4 promotions, 5 announcements) is real content (Sprint 8b/9b, plus Driving Instructors and JP Services added directly by the project owner 2026-07-14, outside any formal sprint), not placeholder — `data/events.json` deliberately holds only 1 event since Sprint 9b, having removed 5 fake sample events and held out 2 real-but-past-dated ones pending updated dates from the project owner. Sprint 8's seed generator can still produce a full-scale placeholder set on demand (`npm run seed:generate`) but is not used against the real `data/` directory now that real content exists. All business photos — and event images — remain a single shared placeholder SVG until real community photography arrives. **Note:** businesses and categories both totaling 18 is a coincidence, not a 1:1 mapping — 12 categories have ≥1 real business, 6 are Sprint 2 placeholder categories (Plumbing, Cleaning, Childcare, Cafés & Restaurants, Builders & Renovations, Pet Services) with zero businesses, kept intentionally per the project owner (2026-07-14) rather than deleted, hidden from `/search`/Popular Categories by Sprint 09b's F-001/F-002 filtering.
