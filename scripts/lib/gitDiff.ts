...
/**
 * Shared git plumbing for the doc-staleness guardrail scripts (Sprint 16,
 * F-006). Each check script only ever deals with parsed diff/commit data
 * from here, never a raw `git` invocation of its own — keeps the actual
 * detection logic in scripts/lib/docChecks.ts testable without touching git.
 */
