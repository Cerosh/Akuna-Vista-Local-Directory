...
describe("validate-doc-freshness run()", () => {
  const closeoutFiles: DiffFile[] = [
    {
      path: "sprints/sprint-13-schofields-weather-dashboard/README.md",
  ...
  it("exits 1 when a sprint closes out without .ai/CONTEXT.md in the same commit", () => {
    const result = run(
      ...
      "feat: add Schofields weather dashboard via Open-Meteo (Sprint 13)",
    );
  ...
  it("exits 0 when .ai/CONTEXT.md is part of the same commit", () => {
    const result = run(
      closeoutFiles,
      ["sprints/sprint-13-schofields-weather-dashboard/README.md", ".ai/CONTEXT.md"],
      "feat: add Schofields weather dashboard via Open-Meteo (Sprint 13)",
    );
