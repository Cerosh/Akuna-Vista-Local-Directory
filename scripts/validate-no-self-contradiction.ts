 ...
 *
 * Sprint 16 F-006, Check 1. Blocks any commit whose added lines contain
 * "not yet committed/deployed"-style text — false by construction, since
 * it's being committed right now. See scripts/lib/docChecks.ts for the
 * full pattern list and the incident history behind it.
 *
 * Runs as a commit-msg hook (not pre-commit) so the `Docs-Deferred:`
 * escape hatch works — needed in practice: this check's first real run
 * flagged documentation *about* the stale-text pattern (e.g. this sprint's
 * own backlog notes quoting the exact text that was found and fixed) as
 * if it were a live claim. scripts/** is exempted outright (see
 * docChecks.ts); anything else that's a legitimate historical/analytical
 * reference uses the trailer instead of being silently allowed everywhere.
 */
