 ...
 *
 * Sprint 16 F-006, Check 4. If this commit adds a "Sprint Status:
 * ...Complete/Deployed/✅" line to a sprint's own README.md — the exact
 * moment a sprint closes out — .ai/CONTEXT.md must be part of the same
 * commit. Traces back to CONTEXT.md being found frozen at "Sprint 11, not
 * yet committed/deployed" while Sprints 12-16 had all already shipped.
 * Bypass with a `Docs-Deferred: <reason>` trailer, same as Check 2. Runs
 * as a commit-msg hook (not pre-commit) specifically so the escape hatch
 * works — pre-commit runs before the commit message exists.
 */
export function run(
