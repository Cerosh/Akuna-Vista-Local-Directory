 ...
 *
 * Sprint 16 F-006, Check 2. If a commit references a sprint (e.g. "Sprint
 * 11", "Sprint 09b") and touches application code, at least one file under
 * that sprint's own sprints/sprint-{NN}- (wildcard) folder must be part of the same
 * commit. Traces back to Sprints 01-08b, where implementation commits
 * never touched the sprint's own docs at all — forward-looking only, since
 * this project only started consistently including "(Sprint NN)" in commit
 * messages from Sprint 09b onward (see scripts/lib/docChecks.ts for the
 * verified detail). Bypass with a `Docs-Deferred: <reason>` trailer in the
 * commit message — visible in `git log`, not a silent skip.
 */
